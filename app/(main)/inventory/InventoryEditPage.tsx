...
export default function InventoryEditPage({ id }: InventoryEditPageProps) {
  ...
  const loadData = async () => {
    try {
      const [item, brandsResult, categoriesResult, productTypesResult] = await Promise.all([
      ...
      // Populate form with existing data
        setFormData({
          ...
          assembly: typeof item.assembly === 'number' ? item.assembly === 1 : !!item.assembly,
  ...
  // Convert boolean to number for assembly before sending to backend
  (cleanData as any).assembly = cleanData.assembly ? 1 : 0;
      await inventoryApi.update(id, cleanData as any);
      router.push(`/inventory/${id}`);
    } catch (error: any) {
      console.error('Failed to update inventory item:', error);
      // If backend returns validation errors as array
      if (error && error.response && Array.isArray(error.response.data)) {
        const backendErrors: Partial<InventoryItemFormData> = {};
        error.response.data.forEach((err: any) => {
          if (err.path && err.path[0]) {
            backendErrors[err.path[0] as keyof InventoryItemFormData] = err.message;
          }
        });
        ...
        setErrors({
          sku: error instanceof Error ? error.message : 'Failed to update item'
        });
      }
    } finally {
      setIsLoading(false);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center space-x-4">
        <Button variant="outline" size="sm" onClick={() => router.push(`/inventory/${id}`)} className="flex items-center space-x-2">
          <ArrowLeft className="h-4 w-4" />
          <span>Back</span>
        </Button>
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Edit Inventory Item</h1>
          <p className="text-gray-600 mt-2">Update inventory item details</p>
        </div>
      </div>
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {Object.values(errors).length > 0 && (
          <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded mb-4">
            {Object.entries(errors).map(([field, message]) => (
              <div key={field}>{field}: {typeof message === 'string' ? message : JSON.stringify(message)}</div>
            ))}
          </div>
        )}
        ...
        <Card>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <SearchableSelect
              label="Courier Type"
              name="courierType"
              value={formData.courierType ? String(formData.courierType) : ''}
              onChange={(value) => {
                setFormData(prev => ({
                  ...prev,
                  courierType: value === '' ? undefined : (value as 'Van' | 'White Glove' | 'Bulky' | undefined)
                }));
                if (errors.courierType) setErrors(prev => ({ ...prev, courierType: undefined }));
              }}
              options={[
                { value: 'Van', label: 'Van' },
        ...
        <div className="flex items-center justify-end space-x-4">
          <Button variant="outline" onClick={() => router.push(`/inventory/${id}`)}>Cancel</Button>
